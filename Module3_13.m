%% module 3 - 7 - single frame video denoising - color
...
NumOFrames = 30; % number of frames to play and denoise
...
singleFrames = zeros(vid.NumFrames, vid.Height, vid.Width, 3);
for t = 1:NumOFrames
...
function Inew = NLMD(Ign, var_noise, noise_scale)
    ...
    Inew = zeros(size(Ign,1), size(Ign,2), 3);
    % padded image
    Ipad = Ign;
    ...
    for row = (1:size(Ign,1)) + pad_size
     for col = (1:size(Ign,2)) + pad_size
             ...
             for rcol = -((res_block-1)/2):((res_block-1)/2)
                 ...
                 for frow = -((comp_patch-1)/2):((comp_patch-1)/2)
                    for fcol = -((comp_patch-1)/2):((comp_patch-1)/2)
                        ...
                        d2_comp = d2_comp + ((Ipad(row+frow, col+fcol, 1) - ...
                            Ipad(row+rrow+frow, col+rcol+fcol, 1))^2);
                        d2_comp = d2_comp + ((Ipad(row+frow, col+fcol, 2) - ...
                            Ipad(row+rrow+frow, col+rcol+fcol, 2))^2);
                        d2_comp = d2_comp + ((Ipad(row+frow, col+fcol, 3) - ...
                            Ipad(row+rrow+frow, col+rcol+fcol, 3))^2);
                    end
                 end
        ...
        Inew(row-pad_size, col-pad_size, 1) = pixel_new;
        ...
        Inew(row-pad_size, col-pad_size, 2) = pixel_new;
        ...
        Inew(row-pad_size, col-pad_size, 3) = pixel_new;
     end
    end
